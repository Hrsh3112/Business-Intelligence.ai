...
def adapt_c3_output(raw: Any, *, original: AnomalyReport) -> EnrichedReport:
    if isinstance(raw, EnrichedReport):
        ...
        try:
            ...
            enriched = EnrichedReport.model_validate(dumped)
    ...
    if enriched.anomaly_report.model_dump(mode="json", by_alias=True) != original.model_dump(mode="json", by_alias=True):
