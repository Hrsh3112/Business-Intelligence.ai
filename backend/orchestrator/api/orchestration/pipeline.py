...
async def run_pipeline(company_input: CompanyInput) -> ApiResponse:
    ...
    try:
        ...
        def _run_c1(input_obj: CompanyInput):
            adapted_input = adapt_company_input_for_c1(input_obj)
            raw_report = c1_callable(adapted_input)
            return adapt_c1_output(raw_report, original_c2_input=input_obj)

        report = await asyncio.wait_for(
            asyncio.to_thread(_run_c1, company_input), timeout=settings.C1_TIMEOUT_S
        )
    except TimeoutError:
        c1_ms = _ms(c1_start)
        logger.warning(
